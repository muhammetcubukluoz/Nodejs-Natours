const { promisify } = require('util'); //promissify method
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.changePasswordAfter(decoded.iat)) {
    return next(
      new AppError('User recentyl changed password! Please log again.', 401),
    );
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: 'Your Password reste token(valid for 10min)',
      message,
    });
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!awaituser.correctPassword(req.body.passwordConfitm, user.password)) {
    return next(new AppError('Your current password is wrong.', 401));
  }

  //3- ıf so, update password
